    ...
    <Col lg="4" md="4" sm="6" className="mb-5">
      <div className="car__item">
        <div className="car__img">
          <img src={`data:image/png;base64,${Buffer.from(bufferData).toString('base64')}`}  alt="" className="h-48 mx-auto mix-blend-" />
        </div>
          ...
          <div className={`${vehicle?.status=='available'?'text-green-500':'text-red-600'} font-medium text-center`}>{vehicle.status.toUpperCase()}</div>
           <div className="car__item-info d-flex align-items-center justify-content-between mt-3 mb-4">
            <span className=" d-flex align-items-center gap-1">
              <i class="ri-motorbike-line"></i> {(vehicle.brand +"  " + vehicle.model).slice(0,20)}
            </span>
            <span className=" d-flex align-items-center gap-1">
